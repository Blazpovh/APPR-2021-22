...
slo.regije.poligoni_kratek= slo.regije.poligoni[,c("NAME_1", "long", "lat", "order", "hole", "piece", "id", "group")]
...
g.regije.delez.map = covid.slo.delezi.dnevno.dft %>%
  filter(datum == z.datum) %>%
  select(regija.covid.oznaka = regija, povprecna_temperatura) %>%
  left_join(regije.slo.covid.dft, by="regija.covid.oznaka") %>%
  select(regija, delez.promili) %>%
...
left_join(Tabela_regije,slo.regije.poligoni,
  by = "NAME_1"
) %>%
  ggplot() +
  geom_polygon(
