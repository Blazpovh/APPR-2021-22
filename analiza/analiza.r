...
title="Povprečna poraba in njena napoved"
plot(c(0,200), c(0,zgornja_meja),yaxt='n',xaxt='n',main=title, type = "n", pch=19,cex.lab=1.2, cex.axis=1.3, cex.main=1.5,xlab = "Dnevi",ylab = "Povprečna poraba")  # setting up coord. system
xlabels=c(1:10)*20
...
legend(5, zgornja_meja, legend=(c("Dejanska poraba","Napovedana poraba")), pch=c(16,16),cex=c(1,1),col=c("red","green"),y.intersp=1.5)
dev.off()



s = 3 #stopnja polinoma
A <- matrix(c(1,0,0,0,1,1,1,1,1,2,4,8,1,3,9,27), nrow = 4)
Tabelca_razred1 <- matrix(Tabela_razred1)
l = nrow(Tabela_razred1)
T1 <- matrix(0,length(Tabela_razred1),1)
T1[1:(s+1)] = povpr_poraba_za_vsak_dan1[1:(s+1)]
for (i in ((s+2):l)) {
  P = solve(T1(A), Tabela_razred1[(i-s-1):(i-1)])
  T1[i,]<- P[1] + P[2]*(s+1)+ P[3]*(s+1)^2 +P[4]*(s+1)^3
}
C <- Tabela_razred1
C_napovedan <- T1
slika_aproksimacije_boljsa <-plot(C, type = "o",pch = 20,col="black")
zg_meja <- max(max(C),max(C_napovedan))
jpeg("slike/prikaz_skupne_natancnosti_napovednega_modela_boljse.jpg")
default <- c(5,2,4,2) + 0.1
par(mar = mar.default + c(0, 4, 0, 0),family="serif") #axis(side, at=, labels=, pos=, lty=, col=, las=, tck=, ...)
title="Povprečna poraba in njena boljša napoved"
plot(c(0,200), c(0,zg_meja),yaxt='n',xaxt='n',main=title, type = "n", pch=19,cex.lab=1.2, cex.axis=1.3, cex.main=1.5,xlab = "Dnevi",ylab = "Povprečna poraba")  # setting up coord. system
xlabels=c(1:10)*20
axis(side=1,at=xlabels,labels=xlabels)
#axis(side=2,at=c(0:5)*20,labels=as.factor(c(0:5)*20))
lines(C, type = "o", col = "red",lwd=2,cex = 1,pch=16)
lines(C_napovedan3, type = "o", col = "green",lwd=2,cex = 1,pch=16)
# lines(c_napovedan1, type = "o", col = "blue",lwd=2,cex = 1,pch=16)
legend(5, zg_meja, legend=(c("Dejanska poraba","Napovedana poraba")), pch=c(16,16),cex=c(1,1),col=c("red","green"),y.intersp=1.5)
dev.off()
# ***************************************************
